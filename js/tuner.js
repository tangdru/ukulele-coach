...
class Tuner {
  ...
  _loop() {
    ...
    if (freq !== -1) {
      ...
      // Snap instantly on a note change (no dragging from the old note's
      // reading); otherwise ease the displayed cents toward each new
      // reading instead of jumping straight to it, which is what was
      // making the needle look jumpy frame-to-frame.
      if (key !== this.lastNoteKey || this.smoothedCents === null) {
      ...
      if (this.onUpdate) {
        this.onUpdate({ freq, ...info, cents: Math.round(this.smoothedCents), nearestString: string });
      }
    } else {
      this.smoothedCents = null;
      this.lastNoteKey = null;
